"""Authentication configuration for WitAI."""
...
def _load_or_create_secret() -> str:
    ...
    try:
        ...
        raise RuntimeError(f"Failed to read JWT secret from {secret_file}. Set AUTH_JWT_SECRET explicitly or fix DEER_FLOW_HOME/base directory permissions so WitAI can read its persisted auth secret.") from exc
    ...
    try:
        ...
        raise RuntimeError(f"Failed to persist JWT secret to {secret_file}. Set AUTH_JWT_SECRET explicitly or fix DEER_FLOW_HOME/base directory permissions so WitAI can store a stable auth secret.") from exc
    return secret

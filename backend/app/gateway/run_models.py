...
class RunCreateRequest(BaseModel):
    ...
    context: dict[str, Any] | None = Field(default=None, description="WitAI context overrides (model_name, thinking_enabled, etc.)")
    ...
    def reject_unsupported_run_options(cls, value: Any, info: ValidationInfo) -> Any:
        ...
        if not is_supported:
            raise PydanticCustomError(
                ...
                "Run option '{option}' is not supported by WitAI",
    ...
    def reject_resumable_streams(cls, value: Any) -> Any:
        ...
        # WitAI already serves, so only an explicit ``True`` requests the unsupported feature.

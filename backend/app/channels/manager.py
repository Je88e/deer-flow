"""ChannelManager — consumes inbound messages and dispatches them to the WitAI agent via Gateway."""
...
BOUND_IDENTITY_REQUIRED_MESSAGE = "Connect this channel from WitAI Settings, complete the in-channel connect step, then send your message again."
BOUND_IDENTITY_UNAVAILABLE_MESSAGE = "Channel connection verification is temporarily unavailable. Please try again later or contact the WitAI operator."
...
def _stream_payload_type(payload: Mapping[str, Any]) -> str:
    ...
    * The shape WitAI's own gateway emits (``runtime/serialization.py``
...
def _is_assistant_stream_type(payload_type: str) -> bool:
    ...
    type contains 'tool'") published every other message type, and WitAI
...
def _accumulate_stream_text(
    ...
    attributed to the assistant.  Nothing in WitAI produces it (the gateway
...
def _channel_storage_user_id(msg: InboundMessage) -> str | None:
    """Resolve the canonical WitAI user id for a channel-triggered message.
    ...
    matches where channel files are staged. Prefer the bound WitAI owner,
...
class ChannelManager:
    """Core dispatcher that bridges IM channels to the WitAI agent.
    ...
    def _resolve_run_params(self, msg: InboundMessage, thread_id: str) -> tuple[str, dict[str, Any], dict[str, Any]]:
        ...
        # ``user_id`` drives WitAI-owned memory, files, and thread buckets.
        # For browser-connected IM channels, prefer the WitAI account that
    ...
    async def _get_bound_identity_rejection(self, msg: InboundMessage) -> _BoundIdentityRejection | None:
        ...
        # enforced at the webhook route by HMAC, and the "sender → WitAI
        ...
        # the binding by provider identity before creating WitAI threads or
    ...
    async def _handle_chat(
        ...
        *,
        ...
        # Look up the existing WitAI thread, creating one if this is the

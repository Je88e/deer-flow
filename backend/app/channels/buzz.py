"""Buzz (Nostr) channel: WitAI as a member of a Buzz workspace relay.
...
_CONNECT_REPLY_TEXT = {
    "success": "Buzz connected to WitAI.",
...
# WitAI's hidden model-context wrappers. These are literal tags this codebase
...
class BuzzChannel(Channel):
    ...
    def _membership_filter(self) -> dict:
        ...
        socket opened covers both that window and relay/WitAI clock skew. Cost of
    ...
    async def handle_relay_frame(self, raw: str) -> None:
        ...
        is not necessarily the WitAI operator on a team-run Buzz relay, and
        ...
        to an attacker's WitAI account through ``/connect``. Both handlers make
    ...
    def _strip_own_mention(self, text: str) -> str:
        ...
        (e.g. "@Alice, @WitAI help"), guessing that the first one is ours risks
        ...
        alone rather than guess. The common single-mention case ("@WitAI
        ...
        if not sep:
            return stripped  # "@WitAI" alone: nothing to strip without losing the whole message
    ...
    async def _attach_connection_identity(self, inbound: InboundMessage) -> InboundMessage:
        ...
        bound WitAI account, and revoking the connection has no runtime effect.
    ...
    async def send(self, msg: OutboundMessage) -> None:
        ...
        DEFENSE IN DEPTH: text carrying one of WitAI's hidden model-context

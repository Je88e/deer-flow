"""Trace-based behavioural tests for WitAI, using Monocle Test Tools.
...
* One **offline example** (``test_assertion_api_example``) loads a recorded
  ...
  trace format and the asserter wiring, not WitAI's behaviour. Treat it as the
...
* Two **live tests** drive the agent end-to-end through ``run_agent`` and assert
  ...
  **explicit opt-in** via ``MONOCLE_LIVE_TESTS=1`` (default off, so a plain run
  never spends tokens or hits the network) and need the WitAI app plus the

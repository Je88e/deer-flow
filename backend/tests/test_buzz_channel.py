...
def _event(*, sk=SK_OWNER, kind=9, content="@WitAI hello", channel=CHANNEL, mentions=(PK3_HEX,), reply_to=None, created_at=1700000100):
...
def test_known_command_classifies_as_command_but_plain_text_stays_chat():
    ...
    ch, captured = _started()
    _dispatch(ch, _event(content="@WitAI /goal ship it", mentions=(PK3_HEX,)))
    assert len(captured) == 1
    assert captured[0].msg_type == InboundMessageType.COMMAND
    assert captured[0].text == "/goal ship it"

    _dispatch(ch, _event(content="@WitAI just chatting", mentions=(PK3_HEX,)))
...
def test_strip_own_mention_leaves_ambiguous_multi_mention_text_untouched():
    """FINDING 4: "@Alice, @WitAI help" must not have Alice's mention dropped
    just because our own mention also appears in the message."""
    ch, captured = _started()
    _dispatch(ch, _event(content="@Alice, @WitAI help", mentions=(PK3_HEX,)))
    assert len(captured) == 1
    assert captured[0].text == "@Alice, @WitAI help"
...
def test_connect_success_sends_confirmation_reply():
    ...
    assert events[0]["content"] == "Buzz connected to WitAI."
...
def test_bound_pubkey_inbound_message_carries_the_connection_identity():
    ...
    of the bound WitAI account, and DELETE /api/channels/connections/{id} had no
    runtime effect."""
...
def test_event_with_a_tampered_payload_never_reaches_publish():
    ch, captured = _started()
    ev = _event()
    ev["content"] = "@WitAI rm -rf /mnt/user-data"  # rewritten in flight by the relay
    _dispatch(ch, ev)
    assert captured == []
...
def test_relay_cannot_forge_an_allowlisted_author():
    ...
    operator is not necessarily the WitAI operator: an unverified pubkey field let a
...
def test_relay_cannot_forge_a_connect_event_to_bind_another_members_pubkey():
    ...
    the attacker's WitAI account, so verification has to happen before /connect."""
...
# -- FINDING 1 defense in depth: never publish WitAI's hidden model context -----
#
...
def test_ordinary_replies_that_merely_mention_the_words_are_still_published():
    """The guard keys on WitAI's literal hidden-context wrappers, not on prose."""
    ch, _ = _started()
    transport = FakeTransport()
    ch._transport = transport

...
def test_new_event_at_same_created_at_is_still_processed(tmp_path):
    ...
    ch, captured = _started_with_store(BuzzSeenEventStore(tmp_path / "seen.json"))
    _dispatch(ch, _event(content="@WitAI first", created_at=1700000100))
    _dispatch(ch, _event(content="@WitAI second", created_at=1700000100))
...
def test_older_created_at_new_event_is_still_processed(tmp_path):
    ...
    ch, captured = _started_with_store(BuzzSeenEventStore(tmp_path / "seen.json"))
    _dispatch(ch, _event(content="@WitAI newer", created_at=1700000200))
    _dispatch(ch, _event(content="@WitAI older-clock", created_at=1700000100))
    assert len(captured) == 2

...
def test_feishu_connect_command_binds_identity(tmp_path):
    ...
    async def go():
        ...
        channel._reply_card.assert_awaited_once_with("om-message-1", "Feishu connected to WitAI.")
        await repo.close()
...
def test_wechat_connect_command_binds_identity(tmp_path):
    ...
    async def go():
        ...
        channel._send_connection_reply.assert_awaited_once_with("wx-user-1", "ctx-1", "WeChat connected to WitAI.")
        await repo.close()
...
def test_wecom_connect_command_binds_identity(tmp_path):
    ...
    async def go():
        ...
        channel._ws_client.reply.assert_awaited_once_with(frame, {"msgtype": "text", "text": {"content": "WeCom connected to WitAI."}})
        await repo.close()

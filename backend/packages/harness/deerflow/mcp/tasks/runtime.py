...
def validate_mcp_task_config_snapshot(extensions_config: ExtensionsConfig) -> None:
    ...
    raise McpTaskConfigurationError(f"MCP task-enabled server configuration changed after Gateway startup ({names}); restart WitAI before using durable task tools")
...
def validate_mcp_task_runtime_configuration(
    *,
    ...
    if not bool(getattr(mcp_tasks_config, "enabled", False)):
        raise McpTaskConfigurationError("MCP task_toolsets are configured, so mcp_tasks.enabled=true is required; WitAI will not silently expose these tools as synchronous calls.")

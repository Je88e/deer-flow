...
# Where WitAI's ``/mnt/user-data`` virtual prefix is materialised inside
...
class E2BSandbox(Sandbox):
    """WitAI Sandbox adapter that delegates to an e2b cloud sandbox.

    Args:
        id: WitAI-side sandbox id (used as cache key in the provider).
    ...
    def sandbox_id(self) -> str:
        """e2b-side sandbox id (different from WitAI's ``self.id`` cache key)."""
    ...
    def _resolve_path(self, path: str) -> str:
        """Map WitAI virtual paths into the e2b sandbox filesystem.

"""``TenkiSandboxProvider`` — WitAI :class:`SandboxProvider` backed by Tenki.

Integrates `Tenki <https://tenki.cloud>`_ cloud sandboxes as a WitAI sandbox
...
# would silently drop a long-running thread's state mid-conversation. WitAI
...
def _bootstrap_script(home_dir: str) -> str:
    """Materialise WitAI's virtual path layout inside the sandbox.
...
class TenkiSandboxProvider(WarmPoolLifecycleMixin[TenkiSandbox], SandboxProvider):
    """Run each WitAI sandbox as a Tenki cloud microVM."""
    ...
    def _create_sandbox(self, sandbox_id: str) -> TenkiSandbox:
        ...
        # Materialise WitAI's virtual path layout under the writable HOME.

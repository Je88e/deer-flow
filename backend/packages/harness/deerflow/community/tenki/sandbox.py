"""``TenkiSandbox`` — WitAI :class:`Sandbox` backed by a Tenki cloud sandbox.
...
# ``/mnt`` is root-owned, so WitAI's ``/mnt/user-data`` virtual prefix is not
...
class TenkiSandbox(Sandbox):
    """WitAI Sandbox adapter that delegates to a live Tenki cloud sandbox.

    Args:
        id: WitAI-side sandbox id (the provider's cache key).
    ...
    def _resolve_path(self, path: str) -> str:
        """Map WitAI virtual paths into the writable sandbox home dir.
    ...
    def execute_command(
        ...
        WitAI passes a bash command *string*; it runs through ``sh -lc``.

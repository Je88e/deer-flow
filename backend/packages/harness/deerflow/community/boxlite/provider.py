"""``BoxliteProvider`` — WitAI :class:`SandboxProvider` backed by BoxLite.

Integrates `BoxLite <https://github.com/boxlite-ai/boxlite>`_ — a daemonless,
OCI-native micro-VM runtime — as a WitAI sandbox backend. See
...
# WitAI's virtual prefixes, materialised on the box rootfs at start so the
...
class _EventLoopThread:
    ...
    BoxLite is async-native and its box handles are loop-affine, while WitAI's
...
class BoxliteProvider(WarmPoolLifecycleMixin[BoxliteBox], SandboxProvider):
    """Run each WitAI sandbox as a BoxLite micro-VM."""
    ...
    def _reconcile_orphans(self) -> None:
        """Adopt WitAI-owned BoxLite boxes left by a previous provider/process.

        BoxLite boxes are discovered by a WitAI-specific name prefix. Adopted
    ...
    def _create_box(self, sandbox_id: str) -> BoxliteBox:
        ...
        async def _make() -> SimpleBox:
            ...
            await box.start()
            # Materialise WitAI's virtual prefixes so file ops resolve natively.

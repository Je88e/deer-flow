"""``BoxliteBox`` — WitAI :class:`Sandbox` backed by a BoxLite micro-VM.

WitAI's ``Sandbox`` contract is synchronous; BoxLite's SDK is async-native and
...
``asyncio.to_thread`` worker WitAI invokes us from.
...
class BoxliteBox(Sandbox):
    ...
    Args:
        id: WitAI-side sandbox id (the BoxLite box id).
    ...
    def _resolve_path(self, path: str) -> str:
        ...
        # so WitAI's virtual paths are used as-is; we only reject traversal.
    ...
    def execute_command(
        ...
        WitAI passes a bash command *string*; BoxLite's ``exec`` takes argv, so

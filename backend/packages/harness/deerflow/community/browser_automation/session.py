...
to the event loop that created them. WitAI tools may be awaited on the
    ...
    def get_session(
        ...
        *,
        ...
        if cdp_url and not allow_unguarded_cdp:
            raise RuntimeError("cdp_url uses a browser context where WitAI cannot enforce its SSRF request guard; set allow_unguarded_cdp: true only for an explicitly trusted local Chrome session")

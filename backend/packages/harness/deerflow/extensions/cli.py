...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        ...
        description="Install and manage trusted Python extensions for this WitAI checkout.",
    )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        if args.command == "install":
            ...
            print(f"Installed and enabled {installed.name} ({installed.distribution}). Restart WitAI to load it.")
            return 0
        if args.command == "upgrade":
            ...
            print(f"Upgraded {installed.name} ({installed.distribution}). Restart WitAI to load it.")
            return 0
        if args.command == "disable":
            name = manager.set_enabled(_name_argument(args), enabled=False)
            print(f"Disabled {name}. Restart WitAI to apply the change.")
            return 0
        if args.command == "enable":
            name = manager.set_enabled(_name_argument(args), enabled=True)
            print(f"Enabled {name}. Restart WitAI to apply the change.")
            return 0
        ...
        if args.command == "remove":
            ...
            print(f"Removed {name}. Restart WitAI to apply the change.")
            return 0
...
def find_project_root() -> Path:
    ...
    if configured:
        ...
        if (candidate / "backend" / "pyproject.toml").is_file():
            return candidate
        raise FileNotFoundError(f"DEER_FLOW_PROJECT_ROOT is not a WitAI checkout: {candidate}")
    ...
    for candidate in candidates:
        ...
        if (candidate / "backend" / "pyproject.toml").is_file():
            return candidate
    raise FileNotFoundError("could not find a WitAI checkout; set DEER_FLOW_PROJECT_ROOT")

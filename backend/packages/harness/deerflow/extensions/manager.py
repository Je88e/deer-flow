"""Operator-facing installation management for packaged WitAI extensions."""
...
class ExtensionManager:
    """Install trusted Python extensions into one WitAI checkout."""
    ...
    def _read_plugins(self) -> tuple[str, list[Any]]:
        if not self.config_path.is_file():
            raise FileNotFoundError(f"WitAI config not found: {self.config_path}")
        ...
        try:
            ...
            raise ValueError("invalid WitAI config YAML") from exc
        ...
        if not isinstance(config, dict):
            raise ValueError("WitAI config root must be a mapping")
...
def _validate_remote_source(source: str) -> None:
    ...
    if not scheme:
        raise ValueError("local path references are not deployable; pass a local directory so WitAI can snapshot it")
    if scheme == "file":
        raise ValueError("file URLs are not deployable; pass a local directory so WitAI can snapshot it")
    if scheme == "ssh":

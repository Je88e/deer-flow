"""Hybrid schema bootstrap for WitAI's application tables.
...
| DB state                                      | Action                                  |
|-----------------------------------------------|-----------------------------------------|
| empty (no WitAI tables)                    | ``create_all`` + ``alembic stamp head`` |
| legacy (WitAI tables, no alembic)          | ``create_all`` (baseline tables only, as backfill) + ``stamp 0001_baseline`` + ``upgrade head`` |
...
WitAI-owned table. ``create_all`` runs first because stamping at
...
def _run_create_all_sync(sync_conn: Any) -> None:
    """Create all WitAI-owned tables on *sync_conn*."""
...
async def _sqlite_lock(engine: AsyncEngine):
    ...
    WitAI. The 30s ``busy_timeout`` plus idempotent revisions cover the

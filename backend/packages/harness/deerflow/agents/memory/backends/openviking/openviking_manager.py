...
class OpenVikingMemoryManager(MemoryManager):
    ...
    WitAI continues to choose when capture and recall occur. The official
    ...
    def add_nowait(
        ...
        *,
        ...
        # Preserve WitAI's existing pre-compaction behavior. This backend
    ...
    def _resolve_scope(
        ...
        if resolved_user != self._config.owner_user_id:
            raise MemoryManagerError(f"OpenViking USER API key is bound to WitAI owner_user_id {self._config.owner_user_id!r}, but this request belongs to {resolved_user!r}. Refusing to share one credential across users.")
...
def _load_official_integration() -> dict[str, Any]:
    try:
        ...
        raise ImportError("The OpenViking memory backend requires langchain-openviking==0.1.0. Install WitAI backend dependencies and retry.") from exc

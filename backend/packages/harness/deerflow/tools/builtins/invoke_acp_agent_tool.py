...
def _build_mcp_servers() -> dict[str, dict[str, Any]]:
    """Build ACP ``mcpServers`` config from WitAI's enabled MCP servers."""
...
def _build_acp_mcp_servers() -> list[dict[str, Any]]:
    ...
    The ACP client expects a list of server objects, while WitAI's MCP helper
...
def _format_invocation_error(agent: str, cmd: str, exc: Exception) -> str:
    ...
    if agent == "mcode":
        return (
            f"{message} Install it with `npm install --global @minimax-ai/code`, run `mcode login`, and restart WitAI so it inherits the updated PATH. "
        ...
        try:
            ...
            async with spawn_agent_process(client, cmd, *args, env=agent_env, cwd=physical_cwd) as (conn, proc):
                ...
                await conn.initialize(
                    ...
                    client_info=Implementation(name="deerflow", title="WitAI", version="0.1.0"),
                )

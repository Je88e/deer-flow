#!/usr/bin/env bash
# chat.sh — Send a message to WitAI and collect the streaming response.
#
# Usage:
...
if [ "$HTTP_CODE" = "000" ] || [ "$HTTP_CODE" -ge 400 ]; then
  echo "ERROR: WitAI is not reachable at ${GATEWAY_URL} (HTTP ${HTTP_CODE})" >&2
  echo "Make sure WitAI is running. Start it with: cd <deerflow-dir> && make dev" >&2
  exit 1
fi
...
if result_messages is not None:
    ...
    for event_type, data_str in events:
        if event_type == "error":
            print(f"ERROR from WitAI: {data_str}", file=sys.stderr)
            sys.exit(1)

...
| `contracts/joint-mode.md` | `joint` 模式专有 contract、`3.5` / `5c` 决策、`resolvedBatchNo` 注入前置条件 | `SKILL.md`, `templates/phase-outputs.md`, `templates/report-template.md`, `schemas/report-schema.md` | 保证 joint 专项 contract、固定输出、报告来源标签与结果计数保持一致 |
| `contracts/delivery.md` | 交付顺序、覆盖确认、硬停止规则 | `SKILL.md`, `scripts/README.md`, `schemas/report-schema.md` | 保证入口级交付 contract、脚本接口说明和最终交付校验使用同一停止语义 |
...
| `templates/phase-outputs.md` | phase / sub-phase 固定进度文案与槽位 | 本文件的 orchestration phase 链路 | 保证技能中的执行顺序与用户可见输出一致 |

...
// `@/env` also exports basePath()/stripBasePath() for the /leadagent prefix
// deployment; these tests never configure a base path, so mirror the
// root-deployment shapes ("" and identity).
rs.mock("@/env", () => ({
  env,
  basePath: () => "",

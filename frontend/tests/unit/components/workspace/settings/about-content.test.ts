...
test("aboutMarkdown heading reflects the package version when env is unset", async () => {
  ...
  // empty or undefined APP_VERSION interpolation (`About WitAI ]` /
  // `About WitAI undefined]`), not just removal of the old literal.
  expect(aboutMarkdown).toContain(`[About WitAI ${APP_VERSION}]`);
});

...
<Callout type="info" emoji="📦">
  沙箱是 Agent 进行文件和命令操作的隔离工作区。它让 WitAI
  能够采取真实行动，而不仅仅是对话。
</Callout>
...
命令在 [E2B](https://e2b.dev) 云端的 micro-VM 中执行。每个线程通过 metadata 绑定到一个独立的 e2b 沙箱，因此即使 Gateway 重启或在多进程之间，同一个 WitAI 线程也始终复用同一个 e2b 沙箱。当达到配置的空闲超时时，e2b 会自动回收云端 VM。
...
<Callout type="info">
  因为 e2b 是远程沙箱，WitAI 的虚拟前缀 `/mnt/user-data`
...
如果配置的 `host_path` 对 gateway 进程不可见，WitAI 会记录错误日志并忽略该挂载。

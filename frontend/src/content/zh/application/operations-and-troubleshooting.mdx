---
title: 运维与排障
description: 本页面涵盖运行 WitAI 应用的操作信息：日志记录、常见问题和维护任务。
---
...
本页面涵盖运行 WitAI 应用的操作信息：日志记录、常见问题和维护任务。

## 日志

WitAI 应用在 `logs/` 目录中写入每个服务的日志：
...
1. 确保 `backend/.deer-flow/` 对运行 WitAI 的进程可写。
...
**原因**：SQLite 支持并发读取，但同一时间只有一个写入者。WitAI 的检查点写入和应用写入会共享同一个 SQLite 数据库文件，多用户并发时可能竞争文件锁。

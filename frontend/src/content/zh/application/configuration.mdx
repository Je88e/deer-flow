---
title: 配置
description: 本页面涵盖 WitAI 应用的所有配置层——`config.yaml`、前端环境变量、`extensions_config.json` 和运行时环境变量。
---
...
本页面涵盖 WitAI 应用的所有配置层——`config.yaml`、前端环境变量、`extensions_config.json` 和运行时环境变量。

## config.yaml

`config.yaml` 是 WitAI 的主要配置文件。所有 Agent 行为、模型选择、工具加载和运行时功能都由它控制。
...
Docker 部署时，`localhost` 指向 WitAI 容器本身。请改用容器能够访问的宿主机地址；例如 Docker Desktop 通常可使用 `http://host.docker.internal:8080/v1`。

<Callout type="warning">
  WitAI 依赖工具调用。请确认 GGUF 模型的 chat template 支持 function/tool calls。
...
WitAI 使用 `database` 配置同时管理 LangGraph 检查点数据和应用数据，例如运行记录、反馈和线程元数据。
...
<Callout type="warning">
  不要为 WitAI 工作区文件引入 MCP filesystem server。它会与 WitAI
  内置文件工具形成路径语义不同的重复能力，让 LLM 行为不稳定。WitAI
...
技能启用状态会反映在 `extensions_config.json` 中。你可以直接编辑它，或通过 WitAI 应用界面进行管理。

## 运行时环境变量

这些变量在 WitAI 进程中设置（通过 `.env`、Docker 环境或 shell）：

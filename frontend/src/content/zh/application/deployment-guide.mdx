---
title: 部署指南
description: 本指南涵盖 WitAI 应用所有支持的部署方式：本地开发、Docker Compose 以及使用 Kubernetes 管理沙箱的生产环境。
---
...
本指南涵盖 WitAI 应用所有支持的部署方式：本地开发、Docker Compose 以及使用 Kubernetes 管理沙箱的生产环境。

## 本地开发部署

本地工作流是运行 WitAI 最快的方式，所有服务作为原生进程在你的机器上运行。
...
在 SQLite 模式下，WitAI 会把 LangGraph 检查点数据和应用数据写入同一个 `deerflow.db` 文件。WAL 模式可以支持并发读取，但 SQLite 同一时间仍然只有一个写入者。多个用户同时运行 Agent 时，检查点写入和应用写入会竞争同一个文件锁，可能触发 `sqlite3.OperationalError: database is locked`。
...
WitAI 应用使用 [Better Auth](https://www.better-auth.com/) 进行会话管理。在生产环境中：

...
WitAI ships with two built-in subagents:
...
In addition to the built-in subagents, WitAI supports delegating to external agents through the **Agent Client Protocol (ACP)**. ACP allows WitAI to invoke agents running as separate processes (including third-party CLI tools wrapped with an ACP adapter).
...
<Callout type="tip">
  ACP agents run as child processes managed by WitAI. They communicate over

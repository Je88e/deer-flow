...
description: The **Model Context Protocol (MCP)** is an open standard for connecting language models to external tools and data sources. WitAI's MCP integration allows you to extend the agent with any tool server that implements the MCP protocol — without modifying the harness itself.
---
...
<Callout type="info" emoji="🔌">
  Model Context Protocol (MCP) lets WitAI connect to any external tool
  server. Once connected, MCP tools are available to the Lead Agent exactly like
  built-in tools.
</Callout>

The **Model Context Protocol (MCP)** is an open standard for connecting language models to external tools and data sources. WitAI's MCP integration allows you to extend the agent with any tool server that implements the MCP protocol — without modifying the harness itself.
...
<Callout type="warning">
  Do not add an MCP filesystem server for WitAI workspace files. WitAI
  ...
  selection and file access behavior unstable. WitAI does not currently
...
When the WitAI server starts, `initialize_mcp_tools()` is called. This connects to all enabled MCP servers, retrieves their tool schemas, and caches the results.
...
This means MCP server changes take effect without restarting the WitAI server.
...
Some MCP servers require OAuth authentication. WitAI's `mcp/oauth.py` handles the OAuth flow for servers that declare OAuth requirements in their capability headers.

When an OAuth-protected MCP server is connected, WitAI will:
...
The OAuth flow is transparent to the Lead Agent — it simply calls the tool, and WitAI handles the authentication.

## Per-user credentials

A single HTTP/SSE MCP server entry can serve several WitAI users, each
authenticated to the remote service with their own credential. Declare a
`user_auth` block mapping WitAI user ids to credential header values:
...
authenticated WitAI user and injects that user's credential into the
...
embedded Gateway resolves the WitAI user's raw id (a UUID). The simplest way
...
`user_auth` binds a credential to a *configured* WitAI user. When the caller
...
- **Through the WitAI App UI**: the extensions panel shows connected MCP servers and lets you enable/disable them.

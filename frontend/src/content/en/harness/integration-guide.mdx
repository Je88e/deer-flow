...
description: WitAI Harness is not only a standalone application. It is a Python library you can import and use inside your own backend, API server, automation system, or multi-agent orchestrator.
---
...
<Callout type="info" emoji="🔌">
  WitAI Harness can be embedded into any Python application. This guide
  covers the integration patterns for using WitAI as a library inside your
  own system.
</Callout>

WitAI Harness is not only a standalone application. It is a Python library you can import and use inside your own backend, API server, automation system, or multi-agent orchestrator.
...
WitAI Gateway is itself a FastAPI application. You can mount it as a sub-application or router:
...
# Mount the WitAI gateway router
...
WitAI Harness is built on LangGraph. The Lead Agent is a standard LangGraph graph. You can compose it with your own LangGraph nodes and graphs:
...
WitAI can expose its agent as an MCP server, allowing other MCP-compatible systems to call it as a tool. Refer to the WitAI repository for MCP server integration examples.

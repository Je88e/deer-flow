...
<Callout type="info" emoji="📦">
  ...
  command-based work. It is what makes WitAI capable of real action, not just
  conversation.
</Callout>
...
WitAI supports four sandbox modes. Choose the one that fits your deployment:
...
Commands run in an [E2B](https://e2b.dev) cloud micro-VM. Each thread is bound to its own sandbox via metadata, so the same WitAI thread keeps reaching the same e2b sandbox even across gateway restarts and across processes. The cloud VM is reaped automatically by e2b once the configured idle timeout elapses.
...
<Callout type="info">
  Because e2b is a remote sandbox, WitAI's `/mnt/user-data` virtual prefix is
...
If the configured `host_path` is not visible to the gateway process, WitAI

---
title: Memory
description: Memory is a runtime feature of the WitAI Harness. It is not a simple conversation log — it is a structured store of facts and context summaries that persist across separate sessions and inform the agent's behavior in future conversations.
---
...
<Callout type="info" emoji="💾">
  Memory lets WitAI carry useful information across sessions. The agent
...
Memory is a runtime feature of the WitAI Harness. It is not a simple conversation log — it is a structured store of facts and context summaries that persist across separate sessions and inform the agent's behavior in future conversations.
...
`langchain-openviking` adapter. This first version supports one WitAI user
...
Use `owner_user_id: default` when WitAI authentication is disabled. Put the
...
WitAI supports two levels of memory:

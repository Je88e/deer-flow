---
title: Configuration
description: "WitAI's configuration system is designed around one goal: every meaningful behavior should be expressible in a config file, not hardcoded in the application. This makes deployments reproducible, auditable, and easy to customize per environment."
---
...
<Callout type="info" emoji="⚙️">
  All WitAI Harness behaviors are driven by <code>config.yaml</code>. One
  file controls which models are available, how the sandbox runs, what tools are
  loaded, and how each subsystem behaves.
</Callout>

WitAI's configuration system is designed around one goal: every meaningful behavior should be expressible in a config file, not hardcoded in the application. This makes deployments reproducible, auditable, and easy to customize per environment.

## Config file location

WitAI resolves `config.yaml` using the following priority order:
...
This pattern is how WitAI achieves pluggability without hardcoding class references.

...
description: This page covers day-to-day operational tasks and solutions to common problems when running WitAI App.
---
...
This page covers day-to-day operational tasks and solutions to common problems when running WitAI App.
...
When you pull a new version of WitAI, the `config_version` in `config.example.yaml` may be higher than your `config.yaml`. To merge new fields without losing your customizations:
...
This usually means SQLite is handling concurrent writes from multiple users or long-running agent runs. SQLite supports concurrent reads, but only one writer can hold the database lock at a time. In WitAI, checkpoint writes and application writes share the same SQLite database file.

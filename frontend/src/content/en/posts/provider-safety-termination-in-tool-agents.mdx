...
description: Why tool calls left in a safety-terminated model response must not be executed, and how to configure provider detectors in WitAI.
date: 2026-05-22
tags:
  - Safety
  - Agents
...
## What WitAI Does at This Boundary
...
When `detectors` is not configured explicitly, WitAI uses the built-in detector set:
...
This default set covers common WitAI paths for OpenAI-compatible providers, Anthropic, and Gemini. It does not treat a normal `finish_reason="tool_calls"` as a safety termination, and it does not fold length truncation such as `length` or `max_tokens` into the safety category.
